...
static gboolean
fu_fresco_pd_device_open (FuUsbDevice *device, GError **error)
{
	GUsbDevice *usb_device = fu_usb_device_get_dev (device);

	/* claim interface */
	if (!g_usb_device_claim_interface (usb_device, 0x0, /* FIXME?? */
					   G_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER, /* FIXME?? */
					   error)) {
...
static gboolean
fu_fresco_pd_device_close (FuUsbDevice *device, GError **error)
{
	GUsbDevice *usb_device = fu_usb_device_get_dev (device);

	/* we're done here */
	if (!g_usb_device_release_interface (usb_device, 0x0, /* FIXME?? */
					     G_USB_DEVICE_CLAIM_INTERFACE_BIND_KERNEL_DRIVER, /* FIXME?? */
					     error)) {
				    ...
				    GError **error)
{
	...
	for (guint16 byte_index = 0; byte_index < 0x1D0; byte_index += 3) {
		if (!fu_fresco_pd_device_set_byte (self,
...
fu_fresco_pd_device_class_init (FuFrescoPdDeviceClass *klass)
{
	...
	klass_usb_device->open = fu_fresco_pd_device_open;
	klass_usb_device->close = fu_fresco_pd_device_close;
}
